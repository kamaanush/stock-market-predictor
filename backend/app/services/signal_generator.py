...
def generate_signal(
    ...
    vwap = safe_float(latest.get("vwap"), close)
    atr = safe_float(latest.get("atr"))

    volume = safe_float(latest.get("volume"))
    average_volume = safe_float(latest.get("average_volume"))
    ...
    if high_volume:
        if bullish_ema and above_vwap:
            ...
            reasons.append("High volume without full confirmation")
    else:
    ...
    if score >= 70:
        ...
        entry_price = round(previous_high, 2)
        ...
        stoploss = round(entry_price - minimum_risk, 2)
        ...
        entry_price = round(previous_low, 2)
